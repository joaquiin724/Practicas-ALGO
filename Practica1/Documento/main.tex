...
\section{Algoritmos}
    ...
    \subsection{Estudio teórico}
        ...
        //peor caso    
           0(n-1) ---> quicksort_lims(T, inicial, k);     <--- O(n/2)
           0(1) ---> quicksort_lims(T, k + 1, final);    <--- O(n/2)
        };
        }
        ...
        \end{lstlisting}
        Para el estudio de la eficiencia se ha ido estudiando cada método por separado. El método llamado insercion
        lims no se tiene en cuenta para la eficiencia ya que solo se usa el método de insercion cuando el problema 
        es de un tamaño menor a un UMBRAL definido en 50.
        Para los otros métodos, el quicksort como tal únicamente realiza la llamada a la función quicksort_lims que es 
        la que realmente hace el trabajo, estudiaremos pues ahora su eficiencia.
        ...
        justo la mediana. La parte de la llamada recursiva, es O(n/2), y la llamada a dividir_qs es O(n), por tanto obtenemos
        ...
        \begin{equation*}
            T(n)=T(n-n+1)+(n-1)n+n-1=T(1)+(n-1)n+n-1 \in O(n²)
        \end{equation*}
        Donde se ve que en el peor de los casos el algoritmo es $O(n²)$ que es lo que ocurre con los string por tener un mayor coste de 
