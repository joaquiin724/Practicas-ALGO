...
void guardarCaminoEnArchivo(const string& nombreArchivo, const vector<int>& camino, const vector<vector<double>>& matriz) {
    ...
    for (int nodo : camino) {
        archivo << nodo << " " << matriz[nodo][0] << " " << matriz[nodo][1] << endl; // Asumiendo que la matriz tiene las coordenadas en la primera y segunda columna
    }
...
int main(int argc, char* argv[]) {
    ...
    guardarCaminoEnArchivo("camino.txt", solucion, matriz);
